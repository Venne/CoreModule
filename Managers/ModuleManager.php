...
class ModuleManager extends Object {
	...
	public function uninstallModule($name, $withDependencies = null, $cleanCache = true)
	{
		...
		// remove tags
		$compiler2 = new \Nette\Config\Compiler();
		$module->compile($compiler2);
		$extensions = $compiler2->getExtensions();
		if(!$this->context->configurator->compiler){
			$this->context->configurator->buildContainer();
		}
		$compiler = $this->context->configurator->compiler;
		foreach($compiler->getExtensions() as $exName=>$extension){
			if(isset($extensions[$exName])){
				foreach($extension->getContainerBuilder()->getDefinitions() as $name2 => $item){
					if(array_key_exists("services", $this->config) && array_key_exists($name2, $this->config["services"])){
						unset($this->config["services"][$name2]);
					}
				}
			}
		}
